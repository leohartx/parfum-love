    ...
    private function isSkuExistsInM2ePro($sku, Ess_M2ePro_Model_Listing_Product $listingProduct)
    {
        ...
        if ($listingOtherCollection->getSize() > 0) {
            ...
            if ($listingProduct->getChildObject()->getAmazonGeneralTemplate()->isGenerateSkuModeNo()) {
                ...
                $message.= Mage::helper('M2ePro')->__('Merchant SKU must be unique the product to be listed.');
        ...
        if (in_array($sku,$queue) || isset($this->skusToCheck[$sku])) {
            ...
            if ($listingProduct->getChildObject()->getAmazonGeneralTemplate()->isGenerateSkuModeNo()) {
    // ->__('The product with the same Merchant SKU is being listed now. SKU must be unique the product to be listed.');
                $this->addListingsProductsLogsMessage(
                    $listingProduct,
            'The product with the same Merchant SKU is being listed now. SKU must be unique the product to be listed.',
                    Ess_M2ePro_Model_Log_Abstract::TYPE_ERROR,
                    Ess_M2ePro_Model_Log_Abstract::PRIORITY_MEDIUM
                );
            }
