...
Product.Configurable.prototype = {
    initialize : function(attributes, links, idPrefix, grid, readonly) {
        ...
        this.onLabelUpdate = this.updateLabel.bindAsEventListener(this); // Update
                                                                            // attribute
                                                                            // label
        this.onDefaultsUpdate = this.updateSuperAttributeDefaults.bindAsEventListener(this);
    ...
    updateSuperAttributeDefaults: function (event) {
         var li = Event.findElement(event, 'LI').up('li.attribute');
         li.attributeObject.super_super_attribute_id = parseInt($$('input:checked[type="radio"][name="'+Event.element(event).name+'"]').pluck('value'));
         this.updateSaveInput();
    },
    ...
    createValueRow : function(container, value) {
        ...
        if((!container.attributeObject.super_super_attribute_id && (!this.valueIsChecked || this.valueIsChecked != container.id)) ||
             container.attributeObject.super_super_attribute_id == value.value_index) 
        {
            this.valueIsChecked = container.id;
            templateVariables.set('is_checked','checked');
        }
        templateVariables.set('html_id', container.id + '_'
                + this.valueAutoIndex);
        templateVariables.set('super_super_attribute_id', value.value_index);
        templateVariables.set('container_id', container.id);
        ...
        var typeValue = $$('input:checked[type="radio"][name="conf'+templateVariables.get('container_id')+'"]').pluck('value'); 
        if(parseInt(typeValue) == value.value_index) {
             container.attributeObject.super_super_attribute_id = parseInt(typeValue);
        }
    },
